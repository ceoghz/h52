	...
	<view class="coupons">
		<view class="coupons-header">
			...
			<view class="coupons-header-main d-flex j-sb">
				<view class="" v-for="(item,index) in tabArr" :key="index" @click="isTab = index">
					<text :style="{color:isTab === index ?'#FA6E39':''}">{{item.name}}</text>
					<p v-if="isTab === index"></p>
				</view>
			</view>
		</view>
			...
			<scroll-view scroll-y='true' style="height: 100%;"> 
				<view class="coupons-list" v-for="(index) in 10" :key="index">
					<view class="coupons-box">
					   <view class="coupons-box-left d-felx flex-column " :style="{background:isTab===1?'#BABABA':''}">
					   ...
					   <view class="coupons-box-right  d-felx flex-column " :style="{background:isTab===1?'#F4F4F4':''}">
							<view class="coupons-box-right-1">新人优惠券</view>
							<view class="coupons-box-right-2">有效期至 2020.08.31 23:59</view>
							<view class="coupons-box-right-3 d-flex a-center j-center" :style="{background:isTab===1?'#BABABA':''}">立即使用</view>
					   </view>
					</view>
				</view>
			</scroll-view>
